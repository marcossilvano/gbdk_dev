...
#define MAX_BATS 16                // max = 27 +player +jet +item1 +item2
#define MAX_BATS_GROUP MAX_BATS/2  // 5 per group
...
#define BAT_DELAY_APPEAR       30
#define BAT_DELAY_APPEAR_BLINK 1
#define BAT_SPEED     1
...
u8 bats_update_group = 0;  // flag to alternate objects SAT order
...
u8 collision_map[(LEVEL_TILES_H+1)*LEVEL_TILES_W];  // +1 --> overscan collision
...
bool check_tile_collision(GameObject* obj) {
    ...
    // y & 0xF8 = how many 8's we have
    // Optimization: 
    // y / 8 (tile) * 32 (tiles_w)
    // if (collision_map[(y >> 3) << 5 | (x >> 3)]) return true;
    // clears the 'remainder' bits (0-7) first, then shifts.
    if (collision_map[((uint16_t)(y & 0xF8) << 2) | (x >> 3)]) return true;
...
u8 level_xpos(u8 level) {
    // XXXXXYYY YY------
    // (byte >> 11) * 8  -->  byte >> 11 << 3  -->  byte >> 8
    // +8  -->  fix Tiled top-left anchor
    return (level_starts[level] >> 8) + 8; 
}

u8 level_ypos(u8 level) {
    // XXXXXYYY YY------
    // (byte >> 6) * 8  -->  byte >> 6 << 3  -->  byte >> 3
    // -8  -->  fix Tiled top-left anchor
    return (level_starts[level] >> 3) - 8;
}

void init_player(void) {
    player.active  = true;
    player.visible = true;
    player.fx = intTofixU(level_xpos(level));
    player.fy = intTofixU(level_ypos(level)); // 1/2 + 1/4 = 3/4
...
void init_bats(void) {
    ...
    for (i = 0; i < MAX_BATS; ++i) {
        ...
        bat->tile_idx = BAT_TILE_IDX + (enemy_level-1)*4;
...
void bat_state_appear(GameObject* bat) {
    --bat->counter;
    if (!bat->counter) {
        bat->counter = BAT_DELAY_APPEAR_BLINK;
...
void move_and_bounce_at_walls(GameObject* bat) {
    u8 collided_x = false;

    bat->x += bat->spx;
    if (collision_map[((uint16_t)(bat->y & 0xF8) << 2) | (bat->x >> 3)]) {
    // if (collision_map[(bat->y & 0xF8) | (bat->x >> 3)]) {
        bat->spx = -bat->spx;
...
void update_batsgroup_sat(void) {
    // [CS] Staggered Update: update enemies in groups at each frame
    // 1. Run through the bats current group in alternate order ASC/DESC

    // 3x grous of 9: 
    //   0-8, 9-17, 18-26
    GameObject* bat = bats + bats_update_group;

    u8 sat_idx = BAT_SAT_START + (bats_update_group << 1);
    if (!bats_sat_order) {
        sat_idx += (MAX_BATS_GROUP << 1) - 2;
    }
    
    i = MAX_BATS_GROUP;
    do {
        draw_bat(bat, sat_idx);
    ...
    // alternate order flag
    bats_sat_order = !bats_sat_order;
}

// [CS] Update each 1/3 of the 27 enemies at each frame
...
void init_gameover(void) {
    ...
    // print game over
    gotoxy((SCREEN_TILES_W - 11)/2+1, 11);
    puts("           ");
    gotoxy((SCREEN_TILES_W - 11)/2+1, 12);
    puts(" GAME OVER ");
    gotoxy((SCREEN_TILES_W - 11)/2+1, 13);
    puts("           ");
...
void reset_collision_map(void) {
    ...
    u16 i = (LEVEL_TILES_H+1) * LEVEL_TILES_W;
    do {
        --i;
        *ptr = 0;
    } while (i);
...
void show_collision_map(void) {
    for (u8 y = 0; y < LEVEL_TILES_H; ++y) {
        for (u8 x = 0; x < LEVEL_TILES_W; ++x) {
            // u8 tile = collision_map[(y << 2) & 0xE0 | (x >> 3)];
            u8 tile = collision_map[y*32 + x];
            set_tile(x, y, tile+1);
        }
    }
}
...
void load_level_compact(u8 level) {
    if (level >= NUMBER_OF_LEVELS) {    // error on level number
        return;
    }
    ...
    u16 offset = level * LEVEL_MAP_BYTE_SIZE;// each 96 bytes are a level

    // each row has 32 tiles and each byte has 8 walls data
    const u8* ptr = level_map8x8 + offset;
    i = LEVEL_MAP_BYTE_SIZE;                 // 96 bytes
    do {
        u8 byte = *ptr;                 // process 8 tiles 8x8
        x = 0;
        process_level_byte(byte, x, y);
        ++ptr;
        
        byte = *ptr;                    // process 8 tiles 8x8
        x += LEVEL_TILE;
        process_level_byte(byte, x, y);
        ++ptr;
...
void init_run(void) {
    ...
    // [CS] We need to create debug helpers (that shows dev info)
    // show_collision_map();
...
void launch_item(GameObject* itm) {
    ...
    do {
        ...
        // limit to 0-11 (consider 16x16 tile = LEVEL_TILES_H/2)
        if (itm->y >= LEVEL_TILES_H/2) itm->y -= 4;  
        //                  map[y * 32 + x]
    } while (collision_map[((uint16_t)itm->y << 5) | itm->x]);
