...
static u8 music_size; // number of patterns in channel music array

typedef struct musicChannel {
    // command execution (in channel)
    u8 wait;         // wait counter for channel
    u8* command_ptr; // pointer to current command in pattern (go through pattern)
    
    // channel music array
    u8** channel_adr; // start address of channel music array (doesn't change)
    u8 pattern_idx;  // index of current pattern in channel music array
} MusicChannel;

MusicChannel music_channels[3] = {{0},{0},{0}};
...
void play_music2(u8*** music_ptr, u8 array_size, u8 bpm) {
    i = 0;
    do {
        // channel music array
        music_channels[i].channel_adr = music_ptr[i];
        music_channels[i].pattern_idx = 0;
        music_size = array_size;

        // commands
        music_channels[i].command_ptr = music_channels[i].channel_adr[0];

        i++;
    } while (i < MUSIC_CHANNELS);
...
// [CS] Memory vs. Computation trade-off. Do the bytes in array should 
// comply with the final PSG form (and difficult to manipulate to create effetcts)
// or should I use a custom storage solution, in order to be able to manipulate
// the frequence and volume, using more CPU?
