...
;dscan.c:121: if(DEVICE_SUPPORTS_COLOR) {
	...
	sub	a, #0x11
	ret	NZ
;c:\gamedev\gbdk\include\gb\gb.h:1934: shadow_OAM[nb].prop=prop;
	xor	a, a
	ld	l, c
	ld	h, a
	add	hl, hl
...
;dscan.c:254: for( n=0; n<40; n++ ) {
	ld	c, #0x00
00107$:
;c:\gamedev\gbdk\include\gb\gb.h:1875: shadow_OAM[nb].tile=tile;
	ld	l, c
;	spillPairReg hl
;	spillPairReg hl
	ld	h, #0x00
...
;	spillPairReg hl
	...
	inc	hl
	ld	(hl), #0x1e
;c:\gamedev\gbdk\include\gb\gb.h:1961: OAM_item_t * itm = &shadow_OAM[nb];
	ld	hl, #_shadow_OAM
	add	hl, de
;c:\gamedev\gbdk\include\gb\gb.h:1962: itm->y=y, itm->x=x;
	xor	a, a
	ld	(hl+), a
	ld	(hl), a
;dscan.c:254: for( n=0; n<40; n++ ) {
...
; ---------------------------------
_main::
;c:\gamedev\gbdk\include\gb\gb.h:799: __asm__("di");
	di
